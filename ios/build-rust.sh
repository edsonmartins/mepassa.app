...
cd "$CORE_DIR"
cargo build --release --target aarch64-apple-ios --no-default-features -p mepassa-core
echo ""

# Build for iOS Simulator (ARM64 - Apple Silicon)
echo -e "${GREEN}Building for iOS Simulator ARM64 (aarch64-apple-ios-sim)...${NC}"
cargo build --release --target aarch64-apple-ios-sim --no-default-features -p mepassa-core
echo ""

# Build for iOS Simulator (x86_64 - Intel)
echo -e "${GREEN}Building for iOS Simulator x86_64 (x86_64-apple-ios)...${NC}"
cargo build --release --target x86_64-apple-ios --no-default-features -p mepassa-core
echo ""
