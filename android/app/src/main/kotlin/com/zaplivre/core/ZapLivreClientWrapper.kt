...
object ZapLivreClientWrapper : ZapLivreClientApi {
    private const val TAG = "ZapLivreClientWrapper"
    ...
    suspend fun initialize(context: Context): Boolean = initializationMutex.withLock {
        ...
        try {
            ...
            val dataDir = File(context.filesDir, "zaplivre_data").apply {
                if (!exists()) {
                    mkdirs()
                }
            }

            Log.i(TAG, "Initializing ZapLivreClient with dataDir: ${dataDir.absolutePath}")
            ...
            if (!secureIdentity.isNullOrBlank()) {
                Os.setenv("ZAPLIVRE_IDENTITY_B64", secureIdentity, true)
            } else {
                Os.unsetenv("ZAPLIVRE_IDENTITY_B64")
            }
    ...
    suspend fun exportIdentity(context: Context): String? = withContext(Dispatchers.IO) {
        ...
        val keyFile = File(context.filesDir, "zaplivre_data/identity.key")
        if (!keyFile.exists()) {
    ...
    suspend fun importIdentity(context: Context, backup: String): Boolean = withContext(Dispatchers.IO) {
        ...
        val dbFile = File(dataDir, "zaplivre.db")
        if (dbFile.exists()) {
            dbFile.delete()
        }
     ...
     */
    fun registerVoipEventCallback(callback: uniffi.zaplivre.FfiVoipEventCallback) {
        try {
     ...
     */
    suspend fun registerVideoFrameCallback(callback: uniffi.zaplivre.FfiVideoFrameCallback) = withContext(Dispatchers.IO) {
        try {
     ...
     */
    override suspend fun getMessageReactions(messageId: String): List<uniffi.zaplivre.FfiReaction> = withContext(Dispatchers.IO) {
        try {
            val c = client ?: throw IllegalStateException("Client not initialized")
    ...
    suspend fun getConversationMedia(
        ...
        mediaType: uniffi.zaplivre.FfiMediaType? = null,
        limit: UInt = 100u
    ): List<uniffi.zaplivre.FfiMedia> = withContext(Dispatchers.IO) {
        try {
            val c = client ?: throw IllegalStateException("Client not initialized")
